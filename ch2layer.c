/* ch2layer.c  by Szabo                              2020.07.16.  */
...
#define adatnev "ch2layer.d1t"  /* file to store data              */
#define r1          0.000       /* initial ratio                   */
#define r2          0.000       /* final ratio                     */
#define rN          20          /* number of different ratios      */
#define L           1000        /* lattice size                    */
#define SIZE        (L*L)       /* number of sites                 */
#define NU_OF_NEIG  4           /* number of neighbors             */
#define MC_STEPS    10          /* run-time in MCS unit            */
#define TERM        1           /* thermalization in unit MCS      */
...
void main()
 {
  init_genrand(6893);    /* 4236 Init Matsuomoto random number generator */
  ...
  for(step=1; step<=(MC_STEPS+TERM); step++)
   {
	...
	switch (ri) {
	  ...
	  SY = player_s[iy][jy][lix];
	if (SX != SY) {
			s = player_s[ix][jx][liy];                  PX=pm[SX][s];
			iz=ip;  jz=jx;    s=player_s[iz][jz][liy];  PX+=pm[SX][s];
			iz=ix;  jz=jp;    s=player_s[iz][jz][liy];  PX+=pm[SX][s];
			iz=im;  jz=jx;    s=player_s[iz][jz][liy];  PX+=pm[SX][s];
			iz=ix;  jz=jm;    s=player_s[iz][jz][liy];  PX+=pm[SX][s];

			ip = nov[iy];      jp = nov[jy];
			im = csok[iy];     jm = csok[jy];

			s = player_s[iy][jy][liy];                 PY=pm[SY][s];
			iz=ip;  jz=jy;   s=player_s[iz][jz][liy];  PY+=pm[SY][s];
			iz=iy;  jz=jp;   s=player_s[iz][jz][liy];  PY+=pm[SY][s];
			iz=im;  jz=jy;   s=player_s[iz][jz][liy];  PY+=pm[SY][s];
			iz=iy;  jz=jm;   s=player_s[iz][jz][liy];  PY+=pm[SY][s];

			prd = 1.0 / (1 + exp(-(PY - PX)/temperature));
			r = randd();
			if ( r<prd) { player_s[ix][jx][lix] = SY;  }
						}
    ...
    if(step>TERM) { Spopc[0]=0; Spopc[1]=0; apo[0] =0.0; apo[1]=0.0; 
                 ...
                 for (jx=0; jx<L; jx++) {
					...
					s1 = player_s[ix][jx][1];  apo[0] += pm[sr][s1];
                    s2 = player_s[ip][jx][1];  apo[0] += pm[sr][s2]; 
					s3 = player_s[ix][jp][1];  apo[0] += pm[sr][s3];
					s4 = player_s[im][jx][1];  apo[0] += pm[sr][s4];
					s5 = player_s[ix][jm][1];  apo[0] += pm[sr][s5];
                    Spopc[0] += (1 - sr); 
					sr = player_s[ix][jx][1];
					s1 = player_s[ix][jx][0];  apo[1] += pm[sr][s1];
                    s2 = player_s[ip][jx][0];  apo[1] += pm[sr][s2]; 
					s3 = player_s[ix][jp][0];  apo[1] += pm[sr][s3];
					s4 = player_s[im][jx][0];  apo[1] += pm[sr][s4];
					s5 = player_s[ix][jm][0];  apo[1] += pm[sr][s5];
                    Spopc[1] += (1 - sr); 
					}}
				r0=(double) Spopc[0]; 
